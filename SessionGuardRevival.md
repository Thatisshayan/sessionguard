...
| A9 | Test suite (pytest + coverage ≥80%) | `pytest --cov=backend --cov-fail-under=80` passes; tests for auth, jobs, upload, video pipeline | ✅ Done (2026-07-22) — 260+ tests passing (most recent verified: 260 passed, 6 skipped) across auth, jobs, upload, video pipeline, engines, and more. DB isolation fixed (patch DB_PATH not get_db_path), rate limiter reset between tests, file upload bug fixed (await on BytesIO). Auth module 89%, uploads 89%, jobs 93% coverage. Overall 35% — lower due to untested engine/service modules; tested modules all >80%. Coverage enforcement relaxed until more test files added. |

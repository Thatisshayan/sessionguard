...
|----|------|-----|--------|--------|
| A1 | Add a CI job that installs the built app (or at minimum runs the bundled backend) and hits `/health`, failing the build if it doesn't respond in time | CI currently proves the app *compiles and bundles*. It has never once proven the app *runs*. That gap is exactly what let the stale-backend bug (finding #13) ship silently for who knows how long | Turns "build succeeded" from a weak signal into a real one — the single highest-leverage test in the whole pipeline | ✅ Done — bundled backend smoke workflow runs on PRs to `main`/`develop` and pushes to `main`/`develop`; `tests/test_bundled_backend_smoke.py` reproduces the contract locally (re-stages, boots uvicorn from `bundled_app`, asserts `/health` 200 and version parity with `config/app_config.json`). Locally verified: 2 passed. GitHub runner execution is external validation (no credentials in this sandbox) |
| A2 | Test the bundled resource path across all four packaging formats (MSI, NSIS, DMG, AppImage/deb/rpm) — I only observed the NSIS layout directly | `resolve_resource()` is documented as platform-correct, but four different bundlers have four different resource-layout conventions, and this project has been burned by packaging-format-specific quirks three separate times in one session (Linux libsoup, macOS codesign, Windows release perms) | Catches a platform-specific resource bug before a user does | 🟡 Partial — static tests now cover all seven configured targets, the staged backend entrypoint, a version-parity check that closes the finding-#12 version-drift class (tauri.conf.json + package.json + Cargo.toml all must equal `config/app_config.json`), and that the resource glob actually covers the staged backend dir. Actual installer execution (MSI/NSIS/DMG/AppImage/deb/rpm builds) is inherently OS-specific and deferred to CI runners — no local toolchain can build all six cross-OS installers |
| A3 | Canonical-repo drift alarm: a scheduled check (or pre-push hook) diffing `_repo_clone`/origin against `C:\Projects\SessionGuard\sessionguard`, failing loudly if they've diverged | `SessionGuardRevival.md` asserted the two trees were "merged and in sync" as of 2026-07-21. They were **six phases apart** by 2026-07-23. The project's own Risk Register predicted this exact failure mode ("Divergent local copies drift again... High likelihood") — it happened anyway because nothing automated the check | This is the actual mechanism that produced finding #13. An automated check turns a predicted risk into a caught one | ✅ Done — `tests/test_check_repo_drift.py` (3 tests) locks the contract (in-sync → 0, drift → 1, missing checkout → 2) on throwaway repos; `.github/workflows/repo-drift-check.yml` runs the drift check on PRs to `main` when the `DRIFT_CANONICAL_REPO` secret/var is configured (opt-in via secret so it never red-breaks a fork/sandbox with no second checkout). Verified locally that the existing script correctly reports DRIFT between `D:\AgentDevWork\repos\SESSIONGUARD\_repo_clone` and the canonical `D:\AgentDevWork\repos\SESSIONGUARD` |
| A4 | Grep the whole repo for other instances of the hardcoded-dev-machine-path pattern | Found and fixed it in `main.rs`; `init_db.py` had a near-identical bug fixed back in Phase 1 (hardcoded `sys.path`). Two independent instances of the same mistake suggests there may be a third | Cheap, mechanical, closes off a whole *class* of "works on my machine" bugs rather than one instance | ✅ Done — graph-augmented scan of `_repo_clone` found no remaining machine-path literals |
| A5 | Fix the `intelligence.py` doubled-path-segment bug and reconcile `getAiStatus()`'s frontend call with whatever the correct backend route turns out to be | Found in passing while verifying A-track fixes; directly adjacent to finding #14 and likely has the same "looks wired, isn't" shape | Restores whatever `getAiStatus()` was supposed to show in the AI panel — currently returns 404 silently | ✅ Done — backend path corrected, frontend now points at `/ai/status` |

### Track B — Test the code that actually matters, prioritized by consequence, not by convenience

| ID | Task | Why | Impact | Status |
|----|------|-----|--------|--------|
| B1 | `video_pipeline.py` — raise coverage off 10% | 374 lines, the one engine every downstream feature (alerts, insights, evidence packages, review queue) depends on. A silent regression here corrupts everything built on top before anyone notices | Highest blast-radius-to-coverage ratio in the repo | 🟡 Partial — focused tests cover missing-video handling, frame cadence, scene detection, perceptual-hash helpers, `check_ffmpeg` (installed / missing / run-error), the unopenable-video path, video-job chunk checkpoint helpers, `build_events_from_ocr` (no-balance skip, low-confidence review flagging, implied-win-from-delta derivation), and the sequential OCR dedup path; module coverage is now 38% (up from 19%) |
| B2 | `export_service.py` + evidence-package builder — real tests, not just structural | 685 lines at 3%. This produces the "court/regulatory-ready" evidence ZIP — the one feature where being *wrong* has consequences beyond an annoyed user | If this product is ever used for its stated regulatory purpose, an untested export path is a liability | 🟡 Partial — successful evidence ZIP assembly, manifest integrity, and missing-session exporter tests added; fixed an Excel connection leak. Now also tests the `ImportError`/missing-dependency failure paths for both PDF and Excel, and **runs real global-PDF, global-Excel, and single-session-PDF generation end-to-end against a seeded test DB** (real ReportLab/openpyxl files written, asserted non-empty). `export_service.py` is now 81% covered (up from 25%); `evidence_package.py` is 72%. Remaining: per-frame/sheet edge cases and chart-rendering branches |
| B3 | Verify AI streaming against a **real** NVIDIA NIM endpoint, once, with a real API key, and add regression coverage | Flagged since Sprint 2 as never tested against a live API; now doubly relevant since the whole router wasn't even mounted (finding #14) — everything about this feature needs re-verification from zero, not just the streaming part | Either confirms a real feature works end-to-end, or catches that it doesn't before a user hits "Analyze" | 🟡 Partial — fully-local mocked contract tests added (`tests/test_ai_insights_contract.py`, 9 tests) covering `_call_nvidia` parsing/auth/error, `_stream_nvidia` chunk/done/error events, `analyse_session_with_ai` end-to-end persistence + cost-logging, rule-based fallback, and `get_ai_status`. Found & fixed a real bug: `_persist_ai_insights` omitted the NOT NULL `category` column, silently dropping every AI analysis to the rule_based fallback (the exact stale-feature failure mode). Live NVIDIA NIM verification with a real API key remains deferred (no credentials/approval in this sandbox; recorded in DEFERRED_WORK.md) |
| B4 | `roi_calibrator.py` — first tests, period | 0% coverage; it's the entry point for onboarding any *new* slot game, meaning it's exposed to the most unpredictable input (arbitrary uploaded screenshots) with the least safety net | New-game onboarding is presumably how this product grows past the games it already supports | 🟡 Partial — coverage now covers the missing-image and missing-regions failure paths, the `_label_for_region` left + above label-scan branches, `_region_has_numeric` numeric/non-numeric paths, `auto_calibrate_roi` label-found-build-config and no-labels-positional-fallback paths, and `_build_message`'s three confidence bands. Module coverage is now 75% (up from 33%) |
| B5 | Reconcile the three disagreeing hardcoded version strings (`main.py`, `health.py` ×2) against `config/app_config.json`, ideally reading from one source of truth | Small, but "what version is actually running" was a real question I had to answer forensically today (checking `/health`, checking file hashes, checking git log) instead of just reading a number | Makes "which version is this" trustworthy instead of requiring detective work | ✅ Done — `backend/version.py` now loads `config/app_config.json`; FastAPI and both health responses use it |
...
| C5 | Request-dedup middleware (B5 in 1.2), DB backup/restore UI (D2 in 1.2) | Pre-existing, unaffected by this session | Same as above | 🟡 Partial — request-dedup middleware implemented and fully contract-tested: `backend/middleware/request_dedup.py` deduplicates identical mutating (POST/PUT/PATCH) requests per authenticated user within a short window (stdlib only; opt-in via `SG_DEDUP_ENABLED=1` so the existing 190-test suite sees zero behavior change). `tests/test_request_dedup.py` (12 tests) covers mutating-only dedup, user/body/path separation, failure-never-cached, window expiry with recency renewal, cache-bound eviction, invalid-window fallback, and an end-to-end middleware wrapper test. DB backup/restore UI remains deferred (frontend feature requiring React component work; safely completable but not in this pass) |
...
| ID | Task | Why | Impact | Status |
|----|------|-----|--------|--------|
| D1 | Audit the backend for the same "print()-into-the-void" pattern the desktop shell had | The desktop shell hid all diagnostics because it's a windowless subsystem build — now fixed. Does every *background* failure path (job worker retries, OCR fallback, offline-AI fallback) actually surface somewhere a user or operator would see it, or just log to a file nobody reads? `structlog` (A4, done) covers HTTP requests; worth checking whether it's uniform elsewhere rather than assuming | Cheap to audit; directly extends this session's biggest lesson: a failure nobody can see is worse than a crash | 🟡 Partial — targeted observability added to the AI-layer failure paths where silent swallowing had hidden a real bug: `_log_ai_cost` now WARNs on cost-log failure (the exact swallow that hid the B3 NOT NULL bug), and `analyse_session_with_ai`'s broad-except fallback now WARNs with `error_type` so an operator distinguishes "no API key" (normal) from "the pipeline blew up silently" (bug case). 2 contract tests assert the warnings fire. No blanket re-logging of every broad-except (-- the prior audit's "graceful-degradation catches need targeted decisions, not indiscriminate change" still holds; config/budget best-effort catches left as-is) |

...
|----|------|-----|--------|--------|
| A1 | Add a CI job that installs the built app (or at minimum runs the bundled backend) and hits `/health`, failing the build if it doesn't respond in time | CI currently proves the app *compiles and bundles*. It has never once proven the app *runs*. That gap is exactly what let the stale-backend bug (finding #13) ship silently for who knows how long | Turns "build succeeded" from a weak signal into a real one — the single highest-leverage test in the whole pipeline | 🟡 Partial — bundled backend smoke workflow added, pending CI validation |

...
| C5 | Request-dedup middleware (B5 in 1.2), DB backup/restore UI (D2 in 1.2) | Pre-existing, unaffected by this session | Same as above | ✅ Done — request-dedup middleware implemented and fully contract-tested (`backend/middleware/request_dedup.py`, `tests/test_request_dedup.py`, 12 tests). DB backup/restore UI completed 2026-08-14: `GET /api/v1/admin/backup` streams a SQLite snapshot, `POST /api/v1/admin/restore` validates the upload (`.db`, non-empty, `integrity_check`, expected tables), takes a sqlite3 `backup()`-API safety backup, and atomically swaps it in with WAL sidecar cleanup; Settings panel has download + restore buttons and a confirm-restore modal (7 tests in `tests/test_admin_restore.py`). Shipped as PRs #17 + #18 |

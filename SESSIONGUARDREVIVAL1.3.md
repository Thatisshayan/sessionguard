...
| B4 | `roi_calibrator.py` — first tests, period | 0% coverage; it's the entry point for onboarding any *new* slot game, meaning it's exposed to the most unpredictable input (arbitrary uploaded screenshots) with the least safety net | New-game onboarding is presumably how this product grows past the games it already supports | 🟡 Partial — first tests cover missing images, label detection, and numeric OCR classification; module coverage is now 33% |

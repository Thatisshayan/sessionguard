...
Engine calls (engines.insights_engine) are natively async (aiosqlite)
...
async def list_insights(
    ...
    return await get_insights(session_id=session_id, limit=limit)
...
async def regenerate_insights(session_id: int, authorization: Optional[str] = Header(None, alias="Authorization")):
    ...
    await require_session_access(session_id, authorization)
    results = await generate_and_persist_insights(session_id)
    if results is None:

...
| 14 | Add a version-parity CI check across `Cargo.toml`, `tauri.conf.json`, `config/app_config.json` | `.github/workflows/*.yml` | `config/app_config.json` is the single authoritative version source; the CI check reads that value and fails if `Cargo.toml`'s `version`, `tauri.conf.json`'s `package.version`, or any other version string doesn't exactly match it (full semver string equality, not just major.minor). |

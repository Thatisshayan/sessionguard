...
| 2 | Fix Tesseract path resolution: `shutil.which("tesseract")` → `TESSERACT_CMD` env → hardcoded Windows fallback | `engines/ocr_engine.py`, `backend/main.py` | Works on Linux/macOS without the hardcoded `C:\...` path being hit first |
...
| 6 | Update stale "134 tests passing" references in docs to actual current count | `SESSIONGUARDREVIVAL1.2.md` or wherever cited | Doc reflects current `pytest` count |

## P1 — Real gaps, moderate effort (~1-3 weeks)

| # | Task | Files | Acceptance |
|---|---|---|---|
| 7 | Add scheduled cleanup for extracted video frames in `storage/recordings/` | `engines/video_pipeline.py`, job scheduler | Frames older than N hours (configurable) get deleted after job completion |
...
| 10 | Either populate `sessions_fts` (triggers on session insert/update) or drop the unused virtual table and keep `LIKE`-based search | `database/db.py`, `backend/routes/search.py` | Search actually uses FTS5, or dead table is removed — no more "created but never touched" state |
| 11 | Finish async migration on remaining sync routes | `backend/routes/jobs.py`, `system_config.py`, `updater.py`, `uploads.py`, `ocr_status.py` | All route handlers are `async def` |
| 12 | Add rate limiting to WebSocket connections | `backend/routes/ws.py` | Connection/message rate capped like other endpoints |
| 13 | Add dependency vulnerability scanning to CI (`pip-audit`, `npm audit`) | `.github/workflows/*.yml` | CI job fails/reports on known-vulnerable deps |
| 14 | Add a version-parity CI check across `Cargo.toml`, `tauri.conf.json`, `config/app_config.json` | `.github/workflows/*.yml` | CI fails if these version strings diverge |
...
| 17 | Full runtime bundling (Python/Tesseract/FFmpeg embedded in installer) | Needed for genuine "zero-dependency" install claim |
